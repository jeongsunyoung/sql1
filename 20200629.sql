...
SELECT ROWNUM, empno, ename    -- 아래줄과 같은말
SELECT ROWNUM, a.*
FROM (SELECT empno, ename
      FROM emp
      ORDER BY ename) a;
...
.
.
n page 공식 : (n-1)*pageSize + 1 ~ n * pageSize 
n * 10 - 9 ~ n * 10
(n-1)*10 + 1 ~ n * 10
...
페이징 처리 쿼리 2 page : 11 ~ 20  --정상적으로 동작 안함
SELECT ROWNUM, a.*
FROM (SELECT empno, ename
      FROM emp
      ORDER BY ename) a
...
ROWNUM의 값을 별칭을 통해서 새로운 컬럼으로 만들고 해당 SELECT SQL을 in-line view로
...
SELECT CONCAT('Hello', CONCAT(', ', 'World')) concat,
        SUBSTR('Hello, World', 1, 5) substr,      --SQL 시작 인덱스 : 1번
        LENGTH('Hello, World') length,
        INSTR('Hello, World', 'o') instr,
        INSTR('Hello, World', 'o', 6) instr,     -- 6번째 이후에 있는 인자 중에 나오는 'o' 찾기
        INSTR('Hello, World', 'o', INSTR('Hello, World', 'o')+1) instr, --첫번째 'o' 나온 후 +1
...
SELECT *
FROM emp
WHERE ename = UPPER('smith');   --상수에 적용해서 사용

SELECT * 
FROM emp
WHERE LOWER(ename) = 'smith';   --컬럼에 적용해서 사용하진 않는다
...
SELECT ROUND(105.54, 1) round,   --반올림결과가 소수점 첫번째 자리까지 나오도록 두번째 자리에서 반올림
       ROUND(105.55, 1) round2,
       ROUND(105.55, 0) round3,   --
       ROUND(105.55) round3,      --위와 동일
       ROUND(105.55, -1) round4   --단 두번째인자가 음수이면 해당 자리에서 반올림
FROM dual;

SELECT TRUNC(105.54, 1) trunc,   --반올림결과가 소수점 첫번째 자리까지 나오도록 두번째 자리에서 내림
       TRUNC(105.55, 1) trunc2,
       TRUNC(105.55, 0) trunc3,   --
       TRUNC(105.55) trunc3,      --위와 동일
       TRUNC(105.55, -1) trunc4   --단 두번째인자가 음수이면 해당 자리에서 내림
FROM dual;
...
SYSDATE :
    오라클에서 제공해주는 특수함수
    1. 인자가 없어
    2. 오라클이 설치된 서버의 현재 년, 월, 일, 시, 분, 초 정보를 변환 해주는 함수
...
날짜타임 +- 정수 : 정수를 일자 취급, 정수만큼 미래, 혹은 과거 날짜의 데이트 값을 변환
...
FROM dual;
5. 위 4개 컬럼을 생성하여 다음과 같이 조회하는 쿼리를 작성하세요
...
p.145 실습 fn2
오늘 날짜를 다음과 같은 포맷으로 조회하는 퀴리를 작성하시오
1. 년-월-일
2. 년-월-일 시간(24)-분-초
3. 일-월-년
