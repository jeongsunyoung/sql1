...
방급 입력한 9999번 사번번호를 갖는 사원의 deptno와, job 컬럼의 값을
...
==> UPDATE 쿼리1 실행할 때 안쪽 SELECT 쿼리가 2개가 포함 ==> 비효율적
...
DELETE emp_copy;   --아래와 동일
TRUNCATE TABLE emp_copy;    --복구 안됨
...
DDL (Data Definition[정의] Lanuage) : 데이터를 정의하는 SQL
...
4. FOREIGN KEY
   ...
   emp 테이블에 데이터를 입력할 때 dept 테이블에 존재하지 않는
...
--PRIMARY KEY 제약조건이 deptno에만 걸려있기 때문에 추가 가능
