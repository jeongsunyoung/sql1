...
--동일한 값을 값는 부서번호 ==> 오류 'PK_DEPT_TEST'
...
UNIQUE 제약조건 : 해당 컬럼의 값이 다른 행에 나오지 않도록 (중복되지 않도록)
                 데이터 무결성을 지켜주는 조건
                 (ex : 사번, 학번)
수업시간 UNIQUE 제약조건 명명 규직 : UK_테이블명_해당컬럼명
...
INSERT INTO dept_test VALUES (1, 'ddit', '대전');
...
CREATE TABLE dept_test (
   ...
   /* dname, loc를 결합해서 중복되는 데이터가 없으면 됨
    ...
    아래는 부서명은 동일하지만 loc 정보가 다르기 때문에 dname, loc 조합은
    서로 다른 데이터
    ddit, daejeon
    ddit, 대전 */
    CONSTRAINT uk_dept_test_dname UNIQUE (dname, loc)   --두개 적용
);

dname, loc 컬럼 조합으로 중복된 데이터가 들어가는, 안들어가는지 테스트

dname, loc 컬럼 조합의 값이 동일한 데이터인 경우 
==> 에러(UNIQUE 제약조건에 의해)
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');
INSERT INTO dept_test VALUES (90, 'ddit', 'daejeon');   --에러
...
FOREIGN KEY : 참조키
한 테이블의 컬럼의 값이 참조하는 테이블의 컬럼 값 중에 
존재하는 값만 입력되도록 제어하는 제약조건

즉 FOREIGN KEY 경우 두개의 테이블 간의 제약조건
...
emp_test 테이블에 brown 사원이 1번 부서에 속한 상태에서
1번 부서를 삭제하는 경우
FK의 기본 설정에서 참조하는 데이터가 없어질 수 없기 때문에 에러 발생
...
1. ON DELETE CASCADE : 부모 데이터를 삭제할 경우 참조하고 있는 자식 데이터를 같이 삭제
   (dept_test 테이블의 1번 부서를 삭제하면 1번 부서에 소속된 brown 사원도 삭제)
2. ON DELETE SET NULL : 부모 데이터를 삭제할 경우 
                        참조하는 자식의 데이터의 컬럼을 NULL로 수정

ON DELETE CASCADE 옵션 확인
DROP TABLE emp_test;
...
테이블 컬럼 구조만 복사하고 싶을 때 WHERE절에 항상 FALSE가 되는
조건을 기술하여 생성 가능
...
1. 존재하지 않았던 새로운 컬럼 추가
    ...
    ** 신규로 추가하는 컬럼의 경우 컬럼 순서가 항상 테이블의 마지막
...
테이블 수정명령을 통홰 emp_test 테이블의 emp_no컬럼으로 PRIMARY KEY 제약 생성
ALTER TABLE emp_test ADD CONSTRAINT pk_emp_test PRIMARY KEY (empno);

제약조건을 활성화 / 비활성화 (ENABLE / DISABLE)
ALTER TABLE emp_test DISABLE CONSTRAINT pk_emp_test;

pk_emp_test 비활성화 되었있기 때문에 empno 컬럼에 중복되는 값 입력이 가능
...
--에러 PRIMARY KEY 제약조건에 위배되기 때문에 데이터를 정리 하거나...
...
--WHERE을 안쓰면 CROSS JOIN 


///////

테이블 수정을 통해서 emp 테이블의 empno 컬럼에 PRIMARY KEY 제약조건 추가
ALTER TABLE emp ADD CONSTRAINT pk_emp_test PRIMARY KEY (empno);

테이블 수정을 통해서 emp 테이블의 deptno 컬럼에 FK 제약조건 추가
ALTER TABLE emp ADD CONSTRAINT pk_emp_test PRIMARY KEY (empno);

테이블 수정을 통해서 dept 테이블의 deptno 컬럼에 PRIMARY KEY 제약조건 추가
ALTER TABLE emp ADD CONSTRAINT pk_emp_test PRIMARY KEY (empno);
