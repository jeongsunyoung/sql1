...
DESC emp;
YYYYMMDD ==> 문자열로 8byte, DATE는 7byte 
...
1. DB에서 I/O의 기준은 행단위가 아니라 block 단위
   한건의 데이터를 조회하더라도, 해당 행이 존재하는 block의 전체를 읽는다
 ...
 2. index 접근, index 접근후 table access
    single block io ==> 읽어야 할 행이 있는 데이터 blocck만 읽어서 처리하는 방식
    소수의 몇건의 데이터를 사용자가 조회할 경우, 그리고 조건에 맞는 인덱스가 존재할 경우
    빠르게 응답을 받을 수 있다
...
FROM emp
ORDER BY job;
--인덱스 구성 컬럼이 (전체) null이면 인덱스에 저장이 안됨(인덱스에 없다)
...
--두번째 세번째 인덱스 중에 두번째는 필요 없음, 3번째 인덱스가 2번째 인덱스의 일도 다 함
...
FROM emp
ORDER BY job, enema;
--인덱스 구성 컬럼이 하나라도 null이 아니면 인덱스에 저장이 됨
...
SELECT *
FROM emp, dept
WHERE 20 = dept.deptno  --상수로 바껴 있음
  AND emp.empno = 7788;
...
FROM dept
WHERE 1 = 1;
구문으로 dept_table2 테이블 생성 후 다음 조건에 맞는 인덱스를 생성
...
CREATE INDEX idx_nu_dept_02 ON dept_test2 (dname);

- deptno, dname 컬럼을 기준으로 non-unique 인덱스 생성
CREATE INDEX idx_nu_dept_03 ON dept_test2 (deptno, dname);


2p.111 실습 idx2
실습 idx1에서 생성한 인덱스 삭제하는 DDL문 작성
DROP INDEX idx_u_dept_test2_01;
DROP INDEX idx_nu_dept_02;
DROP INDEX idx_nu_dept_03;
...
1. empno (=)
2. ename (=)
3. deptno(=), empno (LIKE)    -- 선행컬럼은 (=)이 오는게 좋다
4. deptno(=), sal(between)    -- sal(between), deptno(=) 
5. deptno(=)
   empno(=)
6. deptno, hiredate 컬럼으로 구성된 인덱스가 있을 경우 
   table 접근이 필요 없음 -- where절이 없음
...
emp 테이블에 데이터가 5천만거
10, 20, 30 데이터는 각각 50건씩만 존재
40번데이터 4850만건  ==> (데이터 분포가 나쁘다) table full access
...
SYNONYM
SYSTEM 계정에서 GRANT CREATE SYNONYM TO hr;
...
PC22.v_emp ==> v_emp 시노님을 생성

CREATE SYNONYM v_emp FOR PC22.v_emp;
